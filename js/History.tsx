...
export default function History() {
  ...
  useEffect(() => {
    ...
    (async function () {
      ...
      const svgMarkup = history?.modules.size
        ? await graphviz.layout(composeDOT(history.modules), 'svg', 'dot', {
          wasmBinary,
        })
        : '<svg />';
      if (signal.aborted) return; // Check after all async stuff
      ...
      for (const el of $<SVGPathElement>('#graph g.node')) {
        ...
        const refTypes = history?.referenceTypes.get(key);
      ...
      setPane(history?.modules.size ? 'graph' : 'info');
